...
export const temples = pgTable(
  ...
  () => [
    pgPolicy("temples_select_public", {
      for: "select",
      to: "public",
      using: sql`true`,
    }),
    pgPolicy("temples_update_official", {
      for: "update",
      to: authenticatedRole,
      using: sql`is_temple_official(id)`,
      withCheck: sql`is_temple_official(id)`,
    }),
    pgPolicy("temples_admin_all", {
      for: "all",
      to: authenticatedRole,
      using: sql`is_admin()`,
